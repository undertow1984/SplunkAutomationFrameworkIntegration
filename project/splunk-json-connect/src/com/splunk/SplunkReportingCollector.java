...
public class SplunkReportingCollector extends Reporting {
	...
	public String commitSplunk() throws Exception {
		...
		for (int i = 0; i < this.reportCollector.size(); i++) {
			...
			Object oldValue = this.reportCollector.get(i).get("Test");
	...
	public void submitReporting(String testName) {
		...
		this.reporting.put("tests", methodDetails);

		String stepsJson = gson.toJson(this.steps);
		if (stepsJson.contains("Fail")) {
			this.testResults.put("performanceStatus", "Fail");
		} else if (stepsJson.contains("Pass")) {
			this.testResults.put("performanceStatus", "Pass");
		}
		
		this.reporting.put("timeValues", this.timeValues);
		this.reporting.put("testResults", this.testResults);

		this.reportingResults.put("Test", this.reporting);
